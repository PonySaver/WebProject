...
import { getPurchasesByUser, getProductByID, getPurchaseIdByUserAndProduct, deletePurchase, UpdateProducts } from '../service/service';
  ...
  const handelcheckout = async () => {
    try {
      ...
      for (const cartItem of cartItems) {
        ...
        const purchasedQuantity = quantities[productId] || cartItem.quantity;
        ...
        await UpdateProducts(productId, { ...product, quantity: updatedQuantity });

        // Remove the purchase for this product
        const purchaseId = await getPurchaseIdByUserAndProduct(userid, productId);
        await deletePurchase(purchaseId);
    ...
    <div className="w-11/12 m-auto py-10">
      ...
      <section className="flex justify-between items-center space-x-10">
        ...
        <div className="w-[40%] h-fit border rounded p-5 space-y-5">
          ...
          <button className="w-full p-2 bg-gray-800 text-center text-white rounded" onClick={() => handelcheckout()}>
            Check Out
          </button>
        </div>
      </section>
